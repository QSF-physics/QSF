...
namespace QSF
{
	...
	struct Field : _Operator
	{
		...
		Field(Section& settings) :pulses{ Pulses{PulsePrototype{settings, axisName(Ax)}}... },
			delays{ (std::get<Pulses>(pulses).delay_in_cycles * 2.0 * pi / std::get<Pulses>(pulses).omega)... },
			endtimes{ (std::get<Pulses>(pulses).delay_in_cycles * 2.0 * pi / std::get<Pulses>(pulses).omega + std::get<Pulses>(pulses).pulse_time)... }{}
	};

	struct _VectorPotential {};

	template <AXIS Ax, typename ... Pulses>
	struct VectorPotential : _Operator, _VectorPotential
	{
		...
		VectorPotential(Pulses&& ... p) :
			...
			delays{ (p.delay_in_cycles * 2.0 * pi / p.omega)... },
			endtimes{ (p.delay_in_cycles * 2.0 * pi / p.omega + p.pulse_time)... }{}
		VectorPotential(Section& settings) :pulses{ Pulses{PulsePrototype{settings, axisName(Ax)}}... },
			delays{ (std::get<Pulses>(pulses).delay_in_cycles * 2.0 * pi / std::get<Pulses>(pulses).omega)... },
			endtimes{ (std::get<Pulses>(pulses).delay_in_cycles * 2.0 * pi / std::get<Pulses>(pulses).omega + std::get<Pulses>(pulses).pulse_time)... }{}
	};
