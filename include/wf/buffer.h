namespace QSF
{
	...
	template<bool binary, typename... Ts> struct BufferedOutputs: BufferedOutputsBase, TypeBox<Ts...>
	{
		...
		template<MODE M> void init(std::string& name, ind atstep= 0)
		{
			...
			logBUFFER("REDUCABLE BUFFER SIZE: %tdx%td, NORMAL BUFFER SIZE: %tdx%td",
								sizeInBuffer<true>,
								bufferHeight,
								sizeInBuffer<false>,
								bufferHeight);
			...
			if(comp_interval > 0)
				file_dat= IO::fopen_with_check(name + IO::dat_ext,
																			 atstep ? (binary ? "rb+" : "r+") : (binary ? "wb" : "w"));
			if(atstep)
			...
			(
					[&]
					{
						if constexpr(std::is_same_v<returnType, double> ||
												 std::is_convertible_v<returnType, double>)
						{
		...
		inline void reduceLine()
		{
			if(rbufferSize > 0)
			{
				if(!MPI::pID)
					...
					MPI_Reduce(rbuffer + rbufferCurrentLine,
										 rbuffer + rbufferCurrentLine,
										 sizeInBuffer<true>,
										 MPI_DOUBLE,
										 MPI_SUM,
		...
		void test()
		{
			logTestFatal(log_interval >= comp_interval,
									 "log_interval (%d) should be greater than comp_interval (%d)",
									 log_interval,
									 comp_interval);
			logTestFatal(log_interval % comp_interval == 0,
