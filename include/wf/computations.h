...
template<typename... T> struct TypeBox
{
	static constexpr auto size= sizeof...(T);
	using indices							= n_seq_t<sizeof...(T)>;

	template<size_t N> using type= getType<N, T...>;
};

template<typename RET, bool REDUCE, typename... Ops> struct COMPUTATION: _COMPUTATION
{
	// 14.07 this is not necessary any longer
	static_assert(
			((Ops::rep) | ... | REP::NONE) != REP::BOTH,
	...
	static_assert(std::is_convertible_v<returnType, double> ||
										sizeof(returnType) % sizeof(double) == 0,
								"The size of returnType is not a multiple of double.");

	static constexpr bool reduce= REDUCE;

	static constexpr size_t returnTypeSize=
			std::is_convertible_v<returnType, double> ? 1 : (sizeof(returnType) / sizeof(double));
	...
	template<typename Op> static constexpr bool holdsOp= (std::is_same_v<Op, Ops> || ... || false);
	template<typename Op> static constexpr size_t bufferIndexOp= Index_v<Op, Ops...> * returnTypeSize;
