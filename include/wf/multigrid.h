			...
			: LocalGrid<Hamiltonian, BaseGrid, Components, MPI_GC, MPI::Slices, false>
	{
		...
		void initFFTW_partial()
		{
			if(!mcomm.isMain)
			{
				if(mpiFFTW)
				{
					...
					for(int i= 0; i < 2; i++)
						mpi_plans_partial[i]= fftw_mpi_plan_many_dft(1,
																												 n,
																												 m / n[0],	 // rank, dims, howmany
				...
				for(DIMS i= 1; i <= DIM; i++)
				{
					if(i == DIM || !mcomm.bounded[i])
					{
						if(fftw_rank)
						{
							...
							{
								dims[j]= {
										n_lx[start_dim + j], strides_lx[start_dim + j], strides_lx[start_dim + j]};
								// __logMPI("PARTIAL_FFTW[1]: pID %d region %d start_dim %d j %d
								// n_lx[start_dim+j]=%td strides_lx[start_dim+j]=%td\n", MPI::pID, MPI::region,
								// start_dim, j, n_lx[start_dim + j], strides_lx[start_dim + j]);
							}
							...
							for(int j= 0; j < DIM; j++)
							{
								if(j < start_dim || (j >= start_dim + fftw_rank && j < DIM))
								{
									...
									index++;
									// __logMPI("PARTIAL_FFTW[2]: pID %d region %d start_dim %d index %d j %d
									// n_lx[j]=%td dist[j]=%td\n", MPI::pID, MPI::region, start_dim, index, j,
									// n_lx[j], strides_lx[j]);
								}
							};
							for(int k= 0; k < 2; k++)
								plans_partial[plans_partial_count + DIM * k]=
										fftw_plan_guru64_dft(fftw_rank,
																				 dims,
																				 howmany_rank,
																				 howmany_dims,
																				 reinterpret_cast<fftw_complex*>(psi),
							...
							delete[] howmany_dims;
							plans_partial_count++;
							// __logMPI("PARTIAL_FFTW[3]: pID %d region %d mpiFFTW %d plans_partial_count %d dim
							// %d/%d fftw_rank %d howmany_rank %d\n", MPI::pID, MPI::region, mpiFFTW,
							// plans_partial_count, start_dim, i - 1, fftw_rank, howmany_rank);
						}
						fftw_rank= 0;
					}
			...
			// plans_partial_count); _logMPI("Region %d MPI::pID: %d got %td nodes or %td rows [start row:
			// %td end row: %td]", MPI::region, MPI::pID, m_l, n_lx[0], pos_lx.first, pos_lx.last);
		}
		...
		void init()
		{
			...
			__logMPI("[Region:%d] [freeCoord:%s] [pID:%d]\n",
							 MPI::region,
							 axisName(mcomm.freeCoord).c_str(),
							 MPI::pID);
			int index= 0;
			for(int i= 0; i < MPI::regionCount; i++)
			{
				if(mcomm.freeCoordsCount[MPI::region] - mcomm.freeCoordsCount[i] == 1)
				{
					...
					__logMPI("[Region:%d] getting %s from [pID:%d]\n",
									 MPI::region,
									 axisName(mcomm.freeCoord ^ mcomm.freeCoords[i]).c_str(),
									 index * MPI::rSize + MPI::rID);
					index++;
				}
			}
			// Used for normalization after partial FFT done two times
			partial_inv_m= 1.0;
			for(DIMS i= 0; i < DIM; i++) partial_inv_m*= mcomm.bounded[i] ? inv_n[i] : 1.0;

			// _logMPI("[group %d region %d pID %d] has %td sources, [isMain:%d]", MPI::group,
			// MPI::region, MPI::pID, sourceRegions.size(), mcomm.isMain);
			...
			if(mcomm.lessFree)
				MPI_Win_create(MPI_BOTTOM, 0, sizeof(cxd), MPI_INFO_NULL, mcomm.lessFree, &lessFreeWin);

			if(mcomm.moreFree)
				MPI_Win_create(
						psi, m_l * sizeof(cxd), sizeof(cxd), MPI_INFO_NULL, mcomm.moreFree, &moreFreeWin);
		}
		...
		template<uind dirFree, uind... dirs> void initSliceStrides(seq<dirs...>)
		{
			stride_slice[dirFree][DIM - 1]= 1;
			((stride_slice[dirFree][dirs]=
						sliceShape<dirFree, dirs + 1>() * stride_slice[dirFree][dirs + 1]),
			 ...);
			// std::cout << MPI::pID << " dirFree: " << dirFree << " stride_slice[dirFree][..]" <<
			// stride_slice[dirFree][0] << " " << stride_slice[dirFree][1] << " " <<
			// stride_slice[dirFree][2] << " " << std::endl;
		}
		...
		template<uind dirFree, uind... dirs> void initSliceFFTWplans(seq<dirs...>)
		{
			if constexpr(dirFree == 0)
			{
				printf("SLICE_FFTW[0] pID %d MPI initSliceFFTWplans dirFree: [%td] howmany %td\n",
							 ...
							 dirFree,
							 m_slice / n[0]);
				transf_slice[0]= fftw_mpi_plan_many_dft(1,
																								n,
																								m_slice / n[0],
																								FFTW_MPI_DEFAULT_BLOCK,		// block
																								FFTW_MPI_DEFAULT_BLOCK,		// tblock
			...
			else
			{
				...
				fftw_iodim64 dims[1]= {
						{n[dirFree], stride_slice[dirFree][dirFree], stride_slice[dirFree][dirFree]}};
				fftw_iodim64 howmany_dims[DIM - 1];
				int index= 0;
				// if (MPI::pID == 1)
				// std::cout << "---:> pID:" << MPI::pID << " dirFree:" << dirFree << " dims: " << dims[0].n
				// << " " << dims[0].is << std::endl;
				...
				(
						[&]
						{
							if(dirs != dirFree)
							{
								howmany_dims[index]= {sliceShape<dirFree, dirs>(),
																			stride_slice[dirFree][dirs],
																			stride_slice[dirFree][dirs]};
								index++;
								// if (MPI::pID == 1)
								// std::cout << "---> pID:" << MPI::pID << " dirFree:" << dirFree << " dirs:" <<
								// dirs << "howmany_dims: " << howmany_dims[index - 1].n << " " <<
								// howmany_dims[index - 1].is << std::endl;
							}
						}(),
						...);

				transf_slice[dirFree]=
						fftw_plan_guru64_dft(1,
																 dims,
																 DIM - 1,
																 howmany_dims,	 // rank, dims, howmany_rank, howmany_dims
																 reinterpret_cast<fftw_complex*>(slice),
																 reinterpret_cast<fftw_complex*>(slice),
																 FFTW_FORWARD,
																 MPI::plan_rigor);
		...
		template<uind... dirFree> void initSlice(seq<dirFree...>)
		{
			if(!mcomm.isMain)
			{
		...
		template<uind dirFree, uind dir> inline bool slidesIn()
		{
			if constexpr(DIM == 3)
			{
		...
		template<uind dirFree, uind dir> inline constexpr ind sliceShape()
		{
			// transfer X/Z ==> box shift over Y, transfer Y ==> box shift over Z
			if(slidesIn<dirFree, dir>()) return nCAP;
			else
				return Base::template shape<REP::X, dir>();
		}
		...
		constexpr inline auto slice_offset(I i, Args... args) const		// noexcept
		{
			if constexpr(DIM - 1 == dim) return i * stride_slice[dirFree][dim];
				...
				return i * stride_slice[dirFree][dim] + slice_offset<dirFree, dim + 1>(args...);
		}

		template<uind dirFree, uind dir> inline ind slider(ind boxIndex)
		{
			return slidesIn<dirFree, dir>() ? boxIndex * nCAP : 0;
		}

		template<uind dirFree, uind dir> inline constexpr ind sliceLimited()
		{
			if constexpr(dirFree == dir && dirFree) return nCAP;
			else
				return sliceShape<dirFree, dir>();
		}
		template<uind dirFree, uind dir> inline ind top()
		{
			return ((dirFree == dir && dirFree) ? n_lx[dir] - nCAP : 0);
		}

		template<uind dirFree, uind dir> inline ind tslider(ind boxIndex)
		{
		...
		inline void getData(int rank, ind slice_off, ind wf_offset, ind count)
		{
			...
			// MPI::pID, a, slice_off, count, rank, wf_offset); a++;
		...
		void remove_backups() { Base::remove_backups(IO::psi_ext + std::to_string(MPI::region)); }
		...
		// 	MPI_Reduce((MPI::eID) ? psi : MPI_IN_PLACE, psi, m_l, MPI_CXX_DOUBLE_COMPLEX, MPI_SUM, 0,
		// MPI::eComm); 	if (MPI::region == 0) return Base::_save(common_name + "_joined", df); 	if
		// (df.rep == REP::P) fourier<REP::X>();
		// }
		...
		std::string saveIonizedJoined(std::string common_name= "", DUMP_FORMAT df= {.dim= DIM})
		{
			...
			MPI_Reduce((MPI::eID) ? psi : MPI_IN_PLACE,
								 psi,
								 m_l,
								 MPI_CXX_DOUBLE_COMPLEX,
								 MPI_SUM,
			...
			// EXPERIMENTAL
			IO::path ret= "";
			if(MPI::region == 0) { ret= Base::_save_centered(common_name + "_ionized_joined", df); }
			if(!mcomm.isMain)
			{
				Base::template fourier<REP::P>();
				fourier<REP::X>();
		...
		template<uind dirFree, uind... dirs> inline void getBox(int rank, int box, seq<dirs...>)
		{
			// return;
			...
			do {
				...
				if(Axis<dirFree> != AXIS::X || (isBottomX && counters[0] < sizeX))
					getData(
							rank,
							slice_offset<dirFree>(counters[dirs]..., 0),
							Base::template data_offset<REP::X>((counters[dirs] + slider<dirFree, dirs>(box))...,
																								 slider<dirFree, DIM - 1>(box)),
				...
				if(Axis<dirFree> != AXIS::X || (isTopX && counters[0] >= startX))
					getData(
							rank,
							slice_offset<dirFree>((counters[dirs] + top<dirFree, dirs>())...,
																		top<dirFree, DIM - 1>()),
							Base::template data_offset<REP::X>((counters[dirs] + tslider<dirFree, dirs>(box))...,
																								 tslider<dirFree, DIM - 1>(box)),
							sliceLimited<dirFree, DIM - 1>());
		...
		{
			// std::cout << " -->box dispatch" << rank << " " << boxIndex << " fc:" << uind(fc) << " pID"
			// << MPI::pID << std::endl; Same nodes in each region talk to each other
			switch(fc)
			{
			case AXIS::X: getBox<0>(rank, boxIndex, n_seq<DIM - 1>); break;
			case AXIS::Y: getBox<1>(rank, boxIndex, n_seq<DIM - 1>); break;
			case AXIS::Z: getBox<2>(rank, boxIndex, n_seq<DIM - 1>); break;
			default: break;
			}
		}
		...
		inline double mask_correction(Nodes... nodes)
		{
			if constexpr(DIM == 3)
				...
				return 1.0;
			// else return 1.0;
			// onesixth* (invCAP2(j + boxIndex * CAPnodes, k)) + onehalf * (CAP1(j + boxIndex * CAPnodes)
			// + CAP1(k));
		}

		template<uind dirFree, uind... dirs> inline void maskSlice(int boxIndex, seq<dirs...>)
		{
			double mask_value;
			...
			do {
				...
				if(Axis<dirFree> != AXIS::X || (isBottomX && counters[0] < sizeX))
				{
					mask_value= Base::template inv_mask<0>(
							Base::template neg_dist_from_edge<dirFree>(counters[dirFree]));
					if(MPI::group == 1)
						mask_value_corr=
								mask_correction<dirFree, dirs...>(Base::template neg_dist_from_edge<dirs>(
										counters[dirs] + slider<dirFree, dirs>(boxIndex))...);
				...
				if(Axis<dirFree> != AXIS::X || (isTopX && counters[0] >= startX))
				{
					mask_value= Base::template inv_mask<0>(Base::template neg_dist_from_edge<dirFree>(
							counters[dirFree] + top<dirFree, dirFree>()));
					if(MPI::group == 1)
						mask_value_corr=
								mask_correction<dirFree, dirs...>(Base::template neg_dist_from_edge<dirs>(
										counters[dirs] + tslider<dirFree, dirs>(boxIndex))...);
			...
			while(!(... && ((counters[Base::template rev<dirs>]++,
											 counters[Base::template rev<dirs>] <
													 sliceLimited<dirFree, Base::template rev<dirs>>())
													? (false)
													: (counters[Base::template rev<dirs>]= 0, true))));
		}

		template<uind dirFree, uind... dirs> inline void add(int box, seq<dirs...>)
		{
			ind counters[DIM]= {0};
			do {
			...
			while(!(... && ((counters[Base::template rev<dirs>]++,
											 counters[Base::template rev<dirs>] <
													 sliceShape<dirFree, Base::template rev<dirs>>())
													? (false)
													: (counters[Base::template rev<dirs>]= 0, true))));
		}
			...
			for(int gI= 1; gI < MPI::groupCount; gI++)	 // groups
			{
				...
				for(int bI= 0; bI < boxesCount; bI++)		// boxes
				{
					...
					for(int sI= 0; sI < gI; sI++)		// sources
					{
						...
						// std::cout << "pID: " << MPI::pID << " group: " << MPI::group << " region: " <<
						// MPI::region << " gI:" << gI << " bI: " << bI << " sI: " << sI << std::endl;
...
#pragma region FFToverloads
		...
		{
			if(mcomm.boundedCoordDim > 0) Base::multiplyArray(psi, partial_inv_m);
		}

		template<REP R> inline void fourier_partial()
		{
			// printf("multigrid FT %d pID:%d mpiFFTW:%d plans_partial_count:%d isMAIN:%d\n", int(R),
			// MPI::pID, mpiFFTW, plans_partial_count, mcomm.isMain);
...
#pragma region Tests
		...
		void test()
		{
			// logTestFatal(n % MPI::rSize == 0, "Grid length n (%td) should be divisible by the number of
			// MPI region processes (%d) (FFTW reg)", n, MPI::rSize);
			if constexpr(DIM == 1)
			{
				// logTestFatal(m % (MPI::rSize * MPI::rSize) == 0, "Grid length n (%td) should be divisible
				// by the number of MPI region processes squared (%d) (FFTW req)", n, MPI::pSize *
				// MPI::pSize);
			}
			else
			{
				// logTestFatal((m / n) % (MPI::rSize) == 0, "Row size (m/n=%td) should be divisible by the
				// number of MPI processes (%d) (FFTW req)", m / n, MPI::rSize);
			}

			// logTestFatal(n >= Im::n && n >= Src::n, "Grid length n (%td) should be larger than IM grid
			// length Im::n (%td) and previous mode grid length Src::n (%td)", n, Im::n, Src::n);
		}
