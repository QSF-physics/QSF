...
#pragma endregion AvailableOperations
	...
	struct LocalGrid<Hamiltonian, BaseGrid, Components, MPI_GC, MPI::Slices, false>: BaseGrid
	{
		...
		LocalGrid(BaseGrid g)
				: BaseGrid(g), mcomm(), canLeaveTransposed(DIM > 1 && mcomm.isMain), m_l(m / MPI::rSize)
		{
			init();
			logSETUP("m %td m_l %td", m, m_l);
		}
		...
		void initFFTW()
		{
			...
			for(int i= 0; i < 2; i++)
				mpi_plans[i]= fftw_mpi_plan_many_dft(DIM,
																						 n,
																						 1,												 // rank, dims,howmany
...
#pragma region Computations
		...
		template<MODE M, REP R, uind... Is> inline void evolve_(double delta, seq<Is...>)
		{
			...
			while(!(... && ((counters[rev<Is>]++, counters[rev<Is>] < shape<R, rev<Is>>())
													? (counters[DIM]++, false)
		...
		template<REP R, uind dir_avg, class Op, uind... Is> double average_der_(seq<Is...>)
		{
			...
			do {
				res+=
						abs2(counters[DIM]) * (static_cast<Hamiltonian*>(this)->template call<Op, Is...>(
			...
			if(MPI::rSize > 1)								// edge cases for MPI
			{
				if(MPI::rID < MPI::rSize - 1)		// Recieve from higher MPI::rID
				{
					...
					for(ind j= 0; j < rowSize<REP::X>(); j++)
					{
						...
						curr[lastRow + j][0]=
								inv_2dx[0] * imag(conj(psi[lastRow + j]) *
																	(row_after[j] - psi[lastRow - rowSize<REP::X>() + j]));
					}
				}
		...
		void getNeighbouringNodes(cxd* source, ind rowSize, ind rowCount)
		{
			...
			if(MPI::rSize > 1)
			{
				...
				if(MPI::rID < MPI::rSize - 1)		// Send down
					MPI_Send(&(source[(rowCount - 1) * rowSize]),
									 rowSize,
									 MPI_CXX_DOUBLE_COMPLEX,
									 MPI::rID + 1,
									 7,
...
#pragma region Operations
		template<uind... dirs> inline void maskRegion_(seq<dirs...>)
		{
			...
			while(!(... && ((counters[rev<dirs>]++, counters[rev<dirs>] < shape<REP::X, rev<dirs>>())
													? (counters[DIM]++, false)
						...
						for(ind i= 0; i < n_lx[0]; i++)
						{
							for(ind j= 0; j < i; j++)
							{
								for(ind k= 0; k < j; k++)
								{
									...
									psi[regular]+= psi[regular2] + psi[regular3] - psi[switched] - psi[switched2] -
																 psi[switched3];
...
#pragma region IO
		void _load(IO::path input_path, IO::path ext= IO::psi_ext)
		{
			...
			if(df.dim != DIM)
			{
				logError("Dimensionality of the input (%d) file [%s] is not the same as the one choosen "
								 "for evolution (%s)",
								 input_path.c_str(),
								 df.dim,
								 DIM);
			...
			logIO("Header: [dim]=%d, [rep]=%d, [complex]=%d, [n[0]]=%td, [dx[0]]=%g",
						df.dim,
						int(df.rep),
						df.complex,
						n_new[0],
		...
		IO::path _save(IO::path input_path, DUMP_FORMAT df, IO::path ext= IO::psi_ext)
		{
			...
			logIO("Opening [%s] file [%s] to write [%s], header offset [%td]",
						"wb",
						input_path.c_str(),
			...
			MPI_File_set_view(fh,
												offset + MPI::rID * (m_l / Power(df.downscale, DIM)) *
		...
		template<uind... Is> void phaseShiftTrick(seq<Is...>)
		{
			...
			while(!(... && ((counters[rev<Is>]++, counters[rev<Is>] < shape<REP::X, rev<Is>>())
													? (counters[DIM]++, false)
													: (counters[rev<Is>]= 0, true))));
		}

		IO::path
		_save_centered(IO::path path= "", DUMP_FORMAT df= {.dim= DIM}, IO::path ext= IO::psi_ext)
		{
			if(df.rep == REP::P)
			{
		...
		void snapshot(std::string extra_info, DUMP_FORMAT df= {.dim= DIM})
		{
			...
			if(df.rep == REP::X)
			{
				_save_centered("snapshot_" + std::to_string(x_count) + extra_info,
											 df,
			...
			{
				_save_centered("snapshot_" + std::to_string(p_count) + extra_info,
											 df,
											 IO::psi_ext + std::to_string(MPI::region));
				p_count++;
			}
...
#pragma region Tests
		...
		{
			if(!(n[0] % MPI::rSize == 0))
				logWarning("Grid length n[0] (%td) should be divisible by the number of MPI region "
									 "processes (%d) (FFTW reg)",
									 n[0],
									 MPI::rSize);
			...
			if constexpr(DIM == 1)
			{
				if(!(m % (MPI::rSize * MPI::rSize) == 0))
					logWarning("Grid size m=n[0] (%td) should be divisible by the number of MPI region "
										 "processes squared (%d) (FFTW req)",
										 n[0],
										 MPI::pSize * MPI::pSize);
			}
			else
			{
				if(!((m / n[0]) % (MPI::rSize) == 0))
					logWarning("Row size (m/n[0]=%td) should be divisible by the number of MPI processes "
										 "(%d) (FFTW req)",
										 m / n[0],
										 MPI::rSize);
			}
		...
		void testFFTW()
		{
			...
			*/
				...
				ind ret_m_l= fftw_mpi_local_size_1d(n[0],
																						MPI::rComm,
																						FFTW_FORWARD,
																						MPI::plan_rigor,
																						&ret_n0_lx_i,
				...
				if(m_l != ret_m_l)
					logWarning("Expected m_l [%td] different from the one return by fftw_mpi_local_size_1d "
										 "[%td]! Abort.",
										 m_l,
										 ret_m_l);
				if(n_lx[0] != ret_n0_lx_i)
					logWarning("Expected n_lx[0] [%td] different from the one return by "
										 "fftw_mpi_local_size_1d [%td] (input)! Abort.",
										 n_lx[0],
										 ret_n0_lx_i);
				if(n_lx[0] != ret_n0_lx_o)
					logWarning("Expected n_lx[0] [%td] different from the one return by "
										 "fftw_mpi_local_size_1d [%td] (output)! Abort.",
										 n_lx[0],
										 ret_n0_lx_o);
				// TODO: do other tests
			}
