namespace QSF
{
	...
	struct SinPulse: PulsePrototype
	{
		...
		inline double operator()(double time) const { return field * sin(omega * time + phi); }
	};
	struct ConstantPulse: PulsePrototype
	{
		...
		inline double operator()(double time) const { return field; }
	};
	struct ChemPhysPulse: PulsePrototype
	{
		...
		ChemPhysPulse(_FieldConfig p): PulsePrototype(p), env_mult(p.omega / (2.0 * p.ncycles)) {}
	...
	template<typename Pulse> struct ChemPhysEnvelope: Pulse
	{
		...
		inline double operator()(double time) const
		{
			if constexpr(std::is_same_v<Pulse, EmptyPulse>) return sin(env_mult * time);
				...
				return Pulse::operator()(time) * sin(env_mult * time);
		}
	};

	// Ignores FWHM
	template<typename Pulse> struct Sin2Envelope: Pulse
	{
		...
		inline double operator()(double time) const
		{
			if constexpr(std::is_same_v<Pulse, EmptyPulse>) return Power(sin(env_mult * time), 2);
				...
				return Pulse::operator()(time) * Power(sin(env_mult * time), 2);
		}
	};

	template<typename Pulse> struct RampEnvelope: Pulse
	{
		...
		inline double operator()(double time) const
		{
			if constexpr(std::is_same_v<Pulse, EmptyPulse>)
			{
				...
				else if(time > rightEdge)
					return ramp * (Pulse::pulse_time - time);
				else
					return 1.0;
			}
			else
			{
				if(time < leftEdge) return Pulse::operator()(time) * ramp * time;
					...
					return Pulse::operator()(time) * ramp * (Pulse::pulse_time - time);
				else
					return Pulse::operator()(time) * 1.0;
			}
		}
	};
	...
	template<typename Pulse> struct SinEnvelope: Pulse
	{
		static constexpr std::string_view _name= "ENV_";
		static constexpr std::string_view name = join_v<_name, Pulse::name>;
		double sin_pow;
		...
		inline double operator()(double time) const
		{
			if constexpr(std::is_same_v<Pulse, EmptyPulse>) return pow(sin(env_mult * time), sin_pow);
			else
				return Pulse::operator()(time) * pow(sin(env_mult * time), sin_pow);
		}
	};
	template<typename Pulse> struct GaussianEnvelope: Pulse
	{
		...
		GaussianEnvelope(_FieldConfig p)
				: Pulse{p}, gauss_pow(-4.0 * log(2) / Power(p.FWHM_percent * Pulse::pulse_time, 2))
		{
