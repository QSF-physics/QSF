...
template<typename SeqA, typename SeqB> using concat_seq= typename concat<SeqA, SeqB>::type;
template<typename H, typename... T>
struct concat_all: concat_seq<H, typename concat_all<T...>::type>
{};

// concat any number of seq types
template<auto... Ms> struct concat_all<seq<Ms...>>
{
	using type= seq<Ms...>;
};
template<typename... Seqs> using concat_all_seq= typename concat_all<Seqs...>::type;

template<size_t... Is> constexpr bool is_in(size_t i, seq<Is...>) { return ((i == Is) || ...); }
template<typename SEQ, size_t... I> constexpr bool contains_seq(SEQ sq, seq<I...>)
{
	return (is_in(I, sq) && ...);
}
...
namespace uniq_impl
{
	...
	template<auto... Ms, auto N, auto... Ns>
	struct uniq<seq<Ms...>, N, Ns...>
			: std::conditional_t<(... || (N == Ms)), uniq<seq<Ms...>, Ns...>, uniq<seq<Ms..., N>, Ns...>>
	{};
...
namespace seq_gen_impl
{
	...
	template<size_t S, size_t E, typename= std::enable_if_t<(S <= E)>>
	struct seq_gen_from: seq_offset<S, typename seq_gen<E - S>::type>
	{
...
template<template<auto> typename Pred, class SEQ>
using filter_seq= typename filter_impl::filter<Pred, SEQ>::type;
...
template<size_t O, typename SEQ>
using offset_seq_t= typename seq_gen_impl::seq_offset<O, SEQ>::type;
...
namespace detail
{
	template<class, size_t, class> struct accumulate;

	template<size_t... Is, size_t S, size_t H, size_t... Js>
	struct accumulate<seq<Is...>, S, seq<H, Js...>>: accumulate<seq<Is..., S + H>, S + H, seq<Js...>>
	{};

	template<size_t... Is, size_t S> struct accumulate<seq<Is...>, S, seq<>>
	{
		using type= seq<Is...>;
	};
}		// namespace detail
template<typename SEQ> using accumulate= typename detail::accumulate<seq<>, 0, SEQ>::type;

namespace detail
{
	template<class> struct swap_two_indices;

	template<size_t I> struct swap_two_indices<seq<I>>
	{
		using type= seq<I>;
	};

	template<size_t I1, size_t I2, size_t... Is> struct swap_two_indices<seq<I1, I2, Is...>>
	{
		using type= seq<I2, I1, Is...>;
	};
}		// namespace detail
template<typename SEQ> using swap_seq= typename detail::swap_two_indices<SEQ>::type;

namespace detail
{
	template<size_t N, typename SEQ> struct repeat_seq
	{
		template<typename= n_seq_t<N>> struct impl;

		template<size_t... Is> struct impl<seq<Is...>>
		{
			template<size_t> using wrap= SEQ;
...
template<size_t N, typename SEQ> using repeat_seq= typename detail::repeat_seq<N, SEQ>::type;

/* For types */
template<typename T, typename... Ts> struct Index;

template<typename T> struct Index<T>: std::integral_constant<size_t, 0>
{};

template<typename T, typename... Ts> struct Index<T, T, Ts...>: std::integral_constant<size_t, 0>
{};
...
template<typename T, typename... Ts> constexpr size_t Index_v= Index<T, Ts...>::value;

// index in sequence parameter pack
template<size_t T, size_t H, size_t... Is> constexpr size_t index_in_seq()
{
...
template<size_t N, size_t H, size_t... Is> static constexpr size_t nth_seq_elem(seq<H, Is...>)
{
	if constexpr(N == 0) return H;
	else
		return nth_seq_elem<N - 1>(seq<Is...>{});
}
