...
namespace MPI
{
	...
	void Barrier(MPI_Comm com= MPI_COMM_WORLD) { MPI_Barrier(com); }
...
#define _logMPI(...)                                                                               \
	{                                                                                                \
		if constexpr(bool(DEBUG & DEBUG_MPI))                                                          \
		{                                                                                              \
			for(int no= 0; no < MPI::pSize; no++)                                                        \
			{                                                                                            \
				MPIB if(no == MPI::pID) _logColor(__LOG_YELLOW, __VA_ARGS__)                               \
			}                                                                                            \
		}                                                                                              \
	}
#define __logMPI(...)                                                                              \
	{                                                                                                \
		if constexpr(bool(DEBUG & DEBUG_MPI)) { __LOG2S(__VA_ARGS__) }                                 \
	}
...
namespace MPI
{
	...
	template<class MPIDivision, DIMS DIM, AXIS...> struct Regions;

	template<DIMS DIM, AXIS... freeAxes> struct Regions<Slices, DIM, freeAxes...>
	{
		using MPIDivision= Slices;
		static_assert(Power(2, DIM) >= sizeof...(freeAxes),
									"Number of regions must be less than 2^DIM");
		...
		static constexpr inline AXIS freeCoords[sizeof...(freeAxes)]{freeAxes...};

		static constexpr inline int freeCoordsCount[sizeof...(freeAxes)]{freeAxisCount<freeAxes>...};
		...
		Regions()
		{
			...
			// logTestFatal(pSize % regionCount == 0, "Number of MPI processes pSize (%d) should be
			// divisible by the number of regions regionCount (%d)", pSize, regionCount);
			...
			for(DIMS i= 0; i < DIM;
					i++)	 // Determines whether individual directions are bounded (for ease of use)
			...
			_logMPI("[pID %3d] region %d: [rID %3d/%3d] is in group %d [gID %3d/%3d]",
							pID,
							region,
							rID,
							rSize,
			...
			for(int i= 0; i < regionCount; i++) gMembers[freeCoordsCount[i]]++;
			...
			for(int i= 1; i < groupCount; i++)
			{
				...
				logSETUP("Group %d has %d regions and p-leader %d (prev p-members %d)",
								 i,
								 gMembers[i],
								 groupLeader[i],
								 rSize * gMembers[i - 1]);
			...
			{
				if(freeCoordsCount[i] > maxFree) maxFree= freeCoordsCount[i];
				if(freeCoordsCount[i] < minFree) minFree= freeCoordsCount[i];
			}
			...
			if(freeCoordsCount[region] > minFree)
			{
				...
				// minFree %d maxFree %d\n", pID, gMembers[freeCoordsCount[region]], size,
				// gMembers[freeCoordsCount[region] - 1], minFree, maxFree);
	...
	template<DIMS D, class MPIDivision> using SingleRegion= MPI::Regions<MPIDivision, D, AXIS::NO>;

	// Here we merge positive and negative sides of axes into one region
	template<DIMS D, class MPIDivision> struct MultiRegionsReduced;

	template<class MPIDivision>
	struct MultiRegionsReduced<3_D, MPIDivision>: MPI::Regions<MPIDivision,
																														 3_D,
																														 AXIS::NO,
																														 AXIS::X,
																														 AXIS::Y,
	...
	template<class MPIDivision>
	struct MultiRegionsReduced<2_D, MPIDivision>
			: MPI::Regions<MPIDivision, 2_D, AXIS::NO, AXIS::X, AXIS::Y, AXIS::XY>
	{};

	template<class MPIDivision>
	struct MultiRegionsReduced<1_D, MPIDivision>: MPI::Regions<MPIDivision, 1_D, AXIS::NO, AXIS::X>
	{};

	template<typename T, typename F> void buildAndScatter(F& fun, T*& local_v)
	{
		// T* mask;
		// if (!MPI::rID)
		// {
