...
// // STRINGVIEW CONCAT
// // https
// ://stackoverflow.com/questions/38955940/how-to-concatenate-static-strings-at-compile-time
namespace detail
{
	using namespace std;

	template<string_view const&... Strs> struct join
	{
	...
	template<size_t N, string_view const& Str> struct repeat
	{
		// Join all strings into a single array of chars
		static constexpr auto impl() noexcept
		{
	...
	template<size_t... digits> struct to_chars
	{
		static constexpr auto size	 = sizeof...(digits);
		static constexpr char value[]= {('0' + digits)..., 0};
	};

	template<size_t rem, size_t... digits> struct explode: explode<rem / 10, rem % 10, digits...>
	{};

	template<size_t... digits> struct explode<0, digits...>: to_chars<digits...>
	{};

	template<> struct explode<0>: to_chars<0>
	{};
}		// namespace detail
...
template<size_t num>
static constexpr std::string_view num_to_string{detail::explode<num>::value,
																								detail::explode<num>::size};