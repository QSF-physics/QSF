...
// using areCopyConstructible = typename
// std::conjunction<std::is_copy_constructible<Args>...>::type;

namespace detail
{

	template<size_t, class= void, class...> struct getType;

	template<size_t N, class T, class... Rest> struct getType
	{
		static_assert(!is_same_v<T, void>, "Can't get n-th element from empty container");
		using type= typename getType<N - 1, Rest...>::type;
	};

	template<class T, class... Rest> struct getType<0, T, Rest...>
	{
		using type= T;
	};
}		// namespace detail

template<size_t N, typename... Args> using getType= typename detail::getType<N, Args...>::type;

template<template<class> class, template<class...> class, class...> struct filter;
...
template<class T, template<class...> class Template> struct pred: std::false_type
{};
template<template<class...> class Template, class... Args>
struct pred<Template<Args...>, Template>: std::true_type
{};

// template <template <typename> typename Pred, typename... Ts> struct mfilter_all<Pred,
// tuple<Ts...>>
template<template<typename> typename Pred, typename... Ts> struct mfilter_all
{
	using type=
			decltype(tuple_cat(std::conditional_t<Pred<Ts>::value, std::tuple<Ts>, std::tuple<>>{}...));
};
...
{
	if constexpr((sizeof...(Tp)) > 0) ForEach(f, std::make_index_sequence<sizeof...(Tp)>());
}
...
// general type concat
template<typename T, typename...> struct cat
{
	using type= T;
};
...
template<typename... Ts> using cat_t= typename cat<Ts...>::type;
...
// template <class T>
// struct rBufferT : std::conjunction<is_base_of_v<_MPI_REDUCE, T>, !is_base_of_v<_MPI_IMMEDIATE,
// T>> {}; template <class T> struct xBufferT : std::disjunction<!is_base_of_v<_MPI_REDUCE, T>,
// is_base_of_v<_MPI_IMMEDIATE, T>> {};
...
// template<size_t I, typename T, typename Pred, std::enable_if_t<I != 0, int> = 0>
// constexpr T get_element_helper(T const* arr, Pred&& pred) {
// 	return pred(arr[0]) ? get_element_helper<I - 1>(arr + 1, pred) : get_element_helper<I>(arr + 1,
// pred);
// }

// template<typename T, size_t N, typename Pred, size_t ...Is>
// constexpr std::array<T, sizeof...(Is)> filter_array_helper(std::array<T, N> const& arr, Pred&&
// pred, std::index_sequence<Is...>*) { 	return { get_element_helper<Is>(arr.data(), pred)... };
// }
...
// 	return filter_array_helper(arr, std::forward<Pred>(pred),
// (std::make_index_sequence<M>*)nullptr);
// }
...
// 		return tuple{};
// 	else
// 		return apply([](auto...ts) constexpr { return tuple_cat((whenMatches<WHEN...>(ts))...); },
// tup);
// }
