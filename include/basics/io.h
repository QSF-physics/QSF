...
namespace IOUtils
{
	...
	const fspath home_dir{ std::getenv("HOME") };
	const fspath project_dir{ "." };
	const fspath project_name{ STRINGIFY(PROJNAME) };
	const fspath results_dir{ "Results" };
	const fspath results_project_dir = results_dir / project_name;
	fspath target_path;
	...
	void assignAndCreateTargetDir(fspath location)
	{
		if (!MPI::rID)
		{
			target_path = location;
			std::filesystem::create_directories(target_path);
			logIO("Created directory at %s", target_path.c_str());
		}
	}
