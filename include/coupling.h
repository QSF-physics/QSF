...
namespace QSF
{
	...
	as needed */
	template<class ApproxModel, class... Fields> struct CouplingBase;
	...
	struct CouplingBase<DipoleApprox, Fields...>	 //: COMPUTATION<double, false, Fields...>
	{
		...
		inline void precalc(double time) { (std::get<Fields>(fields)(time), ...); }
		...
		template<class F> double getValue()
		{
			if constexpr((std::is_same_v<F, Fields> || ...))
			{
				...
				return -1;	 // TODO: log meaningful error
		}
	};

	template<class... Fields> struct CouplingBase<NoApprox, Fields...>
	{
		std::tuple<Fields...> fields;
	...
	template<class GaugeType, class... Fields> struct DipoleCoupling;

	// Allows for field auto-promotion to vector potential
	template<class... Fields>
	struct DipoleCoupling<VelocityGauge, Fields...>
			: CouplingBase<DipoleApprox,
										 std::conditional_t<std::is_base_of_v<_VectorPotential, Fields>,
																				Fields,
																				PromoteFieldToA<Fields>>...>
	{
		...
		{
			using target=
					std::conditional_t<std::is_base_of_v<_VectorPotential, F>, F, PromoteFieldToA<F>>;
