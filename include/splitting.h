...
/* ::::::::::::::::::: Base Implementation ::::::::::::::::::: */
template<REP first, REP... rest> struct _SplitBase
{
...
template<REP first, REP middle, REP last> struct Split3Base: _SplitBase<first, middle, last>
{
...
template<typename Base, typename... Coeffs> struct _SplitChain
{
	...
	using reps= std::conditional_t<
			Base::canJoin,
			concat_seq<seq<size_t(Base::firstREP)>, repeat_seq<N, typename Base::restREP>>,
			repeat_seq<N, typename Base::fullREP>>;
	...
	template<uind N> static constexpr REP rep= REP(nth_seq_elem<N>(reps{}));
};

template<typename Base, size_t Order> struct MultiProductSplit
{
	...
	template<size_t N, size_t Ns> struct SplitCoeff
	{
		static constexpr double value= 1.0 / N;
	};

	// this is not necessairly equal to the number of real splits
	template<size_t N> using SplitExpander= from_to_t<1, N>;

	template<size_t I, class= SplitExpander<I>, class= ChainExpander> struct Chain;
	template<size_t I, size_t... Js, size_t... Is>
	struct Chain<I, seq<Js...>, seq<Is...>>: _SplitChain<Base, SplitCoeff<I, Js>...>
	{
		static constexpr double value=
				(... * (Is != I ? double(I * I) / (double(I * I) - double(Is * Is)) : 1.0));
	};
