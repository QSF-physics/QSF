template<DIMS DIM> struct borVec
{
	double dat[DIM];

	template<typename... Args> borVec(Args... args): dat{args...}
	{
		static_assert(DIM == sizeof...(Args), "borVec expects #DIM arguments");
		double norm= sqrt(((args * args) + ... + 0.0));
		if(norm > 0.001)
			for(DIMS i= 0; i < DIM; i++) dat[i]/= norm;
	}

	borVec()= default;

	double& operator[](int index) { return dat[index]; }
	double operator[](int index) const { return dat[index]; }
	...
	{
		double res= 0.0;
		for(int i= 0; i < DIM; i++) res+= dat[i] * a[i];
		return res;
	}
