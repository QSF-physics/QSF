...
template<typename Regions, typename FLUX_TYPE> void defineBorder(size_t regionIndex, FLUX_TYPE flx)
{
	if(!MPI::pID)
	{
		for(i= 0; i < n; i++)
		{
			...
			else
			{
				...
				{
					readInd1= (readInd0 + j);
					if constexpr(DIM == 2) mask[readInd1]= flx.border(i - n2, j - n2);
					else
					{
						readInd1= (readInd1)*n;
						for(k= 0; k < n; k++) mask[readInd1 + k]= flx.border(i - n2, j - n2, k - n2);
					}
				}
			}
		}
	...
	MPI_Scatter(mask,
							local_m,
							borVec_MPI,
							&regionBorders[regionIndex * local_m],
							local_m,
							borVec_MPI,
							0,
							MPI::rComm);
...
template<typename PROP, typename... Args> struct CO_FLUXES: COMPUTATION<double, Args...>, _RBUFFER
{
	...
	template<MODE M, REP R, OPTIMS opts> inline static void forerunner() { calcCurrentMap<PROP>(); }
	template<REP R, OPTIMS opt, typename FLUX_TYPE> inline double calc() const noexcept
	{
		...
		double res				 = 0.0;
		for(int i= 0; i < local_m; i++) res+= (regionBorders[offset + i] * curr[i]);
		return res * dV / dx;
	}
};
...
// 				i * flux_interval, TOTAL_N2S[i], TOTAL_N2D_corr[i], TOTAL_N2D_anticorr[i], TOTAL_S2D[i],
// TOTAL_S2CAP[i], TOTAL_D2CAP[i]);
