...
#pragma region Initialization
		explicit SplitPropagator(PropagatorBase pb,
														 HamWF&& wf,
														 std::string source_section= std::string(name))
				: Config(source_section), PropagatorBase(pb), wf(wf)
		{
			...
			if constexpr(M == MODE::RE)
			{
				...
				logSETUP("maxPulseDuration: %g, dt: %g => max_steps: %td",
		...
		// }
		SplitPropagator(std::string source_section= std::string(name),
										std::string config_file		= "project.ini")
				: Config(source_section, config_file), settings(ini.sections[source_section]), wf(settings)
		{
